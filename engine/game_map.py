...
# actually renders with. engine/render.py's render_decorations tints each
# decoration with its own underlying tile_kind's light/dark TILE_VISUALS
# color instead (the same tint the plain tile itself would get), so a
# decorated tile blends fully into the floor around it rather than
# reading as a fixed-brightness patch independent of its surroundings.
...
def item_entity_from_def(idef: ItemDef, x: int = 0, y: int = 0) -> Entity:
    ...
    return Entity(
        ...
        item=ItemEffect(
            ...
            affix_effect=idef.affix_effect,
            affix_potency=idef.affix_potency,
            affix_duration=idef.affix_duration,
            affix_chance=idef.affix_chance,
...
def entity_from_def(edef: EntityDef, x: int = 0, y: int = 0) -> Entity:
    ...
    return Entity(
        ...
        summon_entity_id=edef.summon_entity_id,
        summon_interval=edef.summon_interval,
