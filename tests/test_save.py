...
def test_round_trip_preserves_skill_and_potion_slots(tmp_path):
    catalog, dungeon_registry, overworld_level, quest_defs, encounter_registry = _world()
    clock = GameClock()
    quest_log = create_quest_log(quest_defs)
    engine = _prison_tower_engine(dungeon_registry, catalog, clock, quest_log)
    ...
    engine.player.skill_slots[2] = "second_wind"
    engine.player.potion_slots = [None, "healing", "teleport"]
    ...
    restored = active_engines2[active_key].player
    assert restored.skill_slots == [None, None, "second_wind", None]
    assert restored.potion_slots == [None, "healing", "teleport"]


def test_restore_save_defaults_skill_and_potion_slots_for_an_old_format_save(tmp_path):
    """A save file written before skill_slots/potion_slots existed has no
    such fields - pydantic should fill in the same defaults
    Entity.__init__ uses, rather than erroring, so old saves keep loading."""
    catalog, dungeon_registry, overworld_level, quest_defs, encounter_registry = _world()
    clock = GameClock()
    quest_log = create_quest_log(quest_defs)
    engine = _prison_tower_engine(dungeon_registry, catalog, clock, quest_log)
    ...
    save_to_path(save, path)
    raw = json.loads(path.read_text(encoding="utf-8"))
    del raw["player"]["skill_slots"]
    del raw["player"]["potion_slots"]
    path.write_text(json.dumps(raw), encoding="utf-8")
    ...
    restored = active_engines2[active_key].player
    assert restored.skill_slots == [None, None, None, None]
    assert restored.potion_slots == ["healing", "teleport", None]
...
def test_round_trip_preserves_shadowed_buff(tmp_path):
    """Same generic active_buffs round-trip as vigor/haste above."""
    catalog, dungeon_registry, overworld_level, quest_defs, encounter_registry = _world()
    clock = GameClock()
    quest_log = create_quest_log(quest_defs)
    engine = _prison_tower_engine(dungeon_registry, catalog, clock, quest_log)
    active_engines = {"prison_tower": engine}
    engine.player.fighter.active_buffs["shadowed"] = ActiveEffect(potency=0, turns_remaining=6)
    ...
    restored_fighter = active_engines2[active_key].player.fighter
    assert restored_fighter.active_buffs["shadowed"].potency == 0
    assert restored_fighter.active_buffs["shadowed"].turns_remaining == 6


def test_round_trip_preserves_sure_footed_buff(tmp_path):
    """Same generic active_buffs round-trip as vigor/haste/shadowed above."""
    catalog, dungeon_registry, overworld_level, quest_defs, encounter_registry = _world()
    clock = GameClock()
    quest_log = create_quest_log(quest_defs)
    engine = _prison_tower_engine(dungeon_registry, catalog, clock, quest_log)
    active_engines = {"prison_tower": engine}
    engine.player.fighter.active_buffs["sure_footed"] = ActiveEffect(potency=0, turns_remaining=9)
    ...
    restored_fighter = active_engines2[active_key].player.fighter
    assert restored_fighter.active_buffs["sure_footed"].potency == 0
    assert restored_fighter.active_buffs["sure_footed"].turns_remaining == 9


def test_round_trip_preserves_ironroot_buff(tmp_path):
    """Same generic active_buffs round-trip as vigor/haste/shadowed/sure_footed above."""
    catalog, dungeon_registry, overworld_level, quest_defs, encounter_registry = _world()
    clock = GameClock()
    quest_log = create_quest_log(quest_defs)
    engine = _prison_tower_engine(dungeon_registry, catalog, clock, quest_log)
    active_engines = {"prison_tower": engine}
    engine.player.fighter.active_buffs["ironroot"] = ActiveEffect(potency=0, turns_remaining=14)
    ...
    restored_fighter = active_engines2[active_key].player.fighter
    assert restored_fighter.active_buffs["ironroot"].potency == 0
    assert restored_fighter.active_buffs["ironroot"].turns_remaining == 14
