...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("lurker", 14, 5, 1, "ambusher"),
        ("vulture", 12, 3, 0, "scavenger"),
        ("mimic_flask", 14, 4, 1, "mimic"),
    ],
)
...
def test_load_overworld_real_shipped_content_is_a_pure_stitch_of_its_two_cells():
    ...
    assert len(overworld.dungeon_entrances) == 24  # heartlands' 15 (windbreak_hold retired, folded into farrows_stake) + Northern Steppe's Watch Post and Weeping Cistern + Dust Reach's Fallen Colonnade, Dust Crossing, Cloven Warren, Rootfall Hollow, Tangle Lock, Ragged Camp, and Long Drift
...
def test_load_overworld_dust_reach_is_reachable_from_the_player_start():
    ...
    remaining northwest slot the grid's rectangularity requires. Cragspine
    still has no dungeon_entrance of its own; Dust Reach now has its
    first, Fallen Colonnade (docs/dungeon_bibles/fallen_colonnade.md,
    generated by tools/procgen/wave_function_collapse.py) - future
    procgen-algorithm passes add one more test dungeon each. Dust Reach's
    walkable terrain must still actually connect across the Heartlands
    seam - two independently-authored cells juxtaposed with no automatic
    edge-matching could easily abut into an accidental wall."""
    catalog = load_catalog()
    dungeon_registry = load_dungeon_registry(DUNGEONS_DIR, catalog)
    known_dungeon_ids = set(dungeon_registry)
    ...
    assert {(e.x, e.y, e.dungeon_id) for e in dust_reach.dungeon_entrances} == {
        ...
        (30, 20, "dust_crossing"),
        (130, 15, "cloven_warren"),
        (100, 80, "rootfall_hollow"),
        (60, 10, "tangle_lock"),
        (145, 55, "ragged_camp"),
        (5, 15, "long_drift"),
    }
